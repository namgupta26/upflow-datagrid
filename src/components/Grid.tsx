...
const Grid = ({gridRowsData, gridHeaderData}: {gridRowsData: Array<object>, gridHeaderData: Array<HeaderItemType>}) =>  {  
    ...
    // when new header data is sent from parent
    useEffect(()=>{
        setGridHeaderData(gridHeaderData)
    },[gridHeaderData])

    // when new rows data is sent from parent
    useEffect(()=>{
        setGridAllData(gridRowsData)
    },[gridRowsData])

    // when newRows from parent is set, then render grid rows
    useEffect(()=>{
    ...
    // when on delete icon is clicked on row.
